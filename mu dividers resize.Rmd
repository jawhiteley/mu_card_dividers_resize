...
\def\DivWidth{92mm}    % Width of each divider (originals are 3.620 in, not sure if that includes the border)
\def\DivHeight{72mm}   % Height of each divider (originals are 2.613 in, which is too short for sleeved cards)
\setlength{\fboxrule}{0.25mm}    % width of border around each divider
...
% Cards themselves are 63 x 88 mm (standard poker size).  
...
% Therefore, a reasonable target is 92 mm wide (about the same as large premium sleeves, maybe even a bit less) and 72 mm tall, to allow 5 mm above sleeves.
% Except that my insert from TinkeringPaws only has a vertical clearance of 70 mm, and the core box only has 68 mm clearance. 
% 70 mm fits in the core box at an angle, but I might not want to go much taller.  Or use 72 mm for output here, and just cut mine to 70. ;)
% Borders: 
