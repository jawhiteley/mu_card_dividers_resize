...
params:
  ...
  div_height: 72.5 # in mm: Height of dividers (originals are 66.4 mm, which is too short for sleeved cards)
  div_width:  91.6 # in mm: Width of dividers (originals are 91.9 mm, including a 1 px wide black border)
...
# DIMENSIONS:
# Dividers in original file are 3.620 in wide x 2.613 in tall, including a 1 px border on all sides (91.9 x 66.4 mm; approximately 92 x 66.5 mm in practice when printed).
# Original images (1 / page) are 1500 x 1200 px, scaled to 254 mm wide on the page (according to Inkscape)
# - 1480 x 1176 pixels trimmed: about 250.5 x 199 mm when printed.  Each divider is 541 x 390 px (without the border)
# To reproduce the original width (and resolution) as closely as possible: (254 mm / 1500 px) * 541 px = 91.61 mm | (250.5 / 1480) * 541 = 91.57
# 91.6 mm wide seems to be the closest to the original width.
...
# 70 mm fits in the core box at an angle, but I might not want to go much taller.  
# Smme have requested an additional 1/4-inch, which would be 72.7 mm (2.863 in, including the border).  I can just cut mine to 70, if I want. ;)
# BORDERS: 
...
# 0.25 mm is a little thinner between dividers, but a bit thicker around the outside (keeping all borders at the same width). I think I prefer a slightly thinner border.
# 0.33-0.35 mm would be closer to the original. If the colour is lighter, a thicker border is probably easier to use and less noticeable anyway.
  img_dir: pdf_images
header-includes: |
  \usepackage{xcolor}
