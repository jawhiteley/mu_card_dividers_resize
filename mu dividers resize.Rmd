...
params:
  modular_code: modular_code/    # path to R script with modular code (for a specific set of input files)
...
if ( !is.null(params$modular_code) ) {
  if ( file.exists(params$modular_code) && !dir.exists(params$modular_code) ) # file exists and is not a directory
    knitr::read_chunk(params$modular_code)
}
```
```{r, modular-setup, include=FALSE}
...
\renewcommand*{\includegraphics}[2][]{\fcolorbox{`r border_colour`}{white}{\parbox[b][\DivHeight][t]{\DivWidth}{\includegraphicsOG[#1]{#2}}}}
...
```
```{r, modular-processing, include=FALSE}
```
...
if (length(page_breaks) > 0){
  for (i in 1:length(img_files)) {
    if (i %in% page_breaks)  # add a page break before output, if requested.
      cat("\\par \n\\pagebreak")
    if (is.na(img_files[i]))  # add a line break before output, if file is missing (NA).
      cat("\\hspace{\\DivWidth} \n")
    else
