...
if (F) {
  # Manual order - just use the same order as in the data file (notwithstanding Anti-Hero grouping)
  div_sorted <- div_long
} else {
  div_sorted <- div_long |> 
    mutate(
      type = factor(type, levels = c("Hero", "Villain", "Anti-Hero", "Pet", "Other", "Challenge")),
      type.ah = factor(type.ah, levels = c("hero", "villain", "ah.hero", "ah.villain")),
      box = ifelse(type == "Other", box, NA) |> 
        factor(levels = c("Teams", "Campaigns")) # drop other values so they are grouped together.
    ) |> 
    arrange(type, type.ah, box, str_rank(name)) |> 
    filter(!is.na(value)) |> 
    # derive page breaks
    mutate(
