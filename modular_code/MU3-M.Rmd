...
# Sort divider data to desired order ====
sort_divs <- "by box"
...
} else if (str_detect("by box, KSE", sort_divs)) {
  # Group all Retail & KSE content together, then by box; sort by type, then name
  div_sorted <- div_long |> 
    ...
    mutate(
      # set sort order with order of levels = (don't forget any!)
      type.ah = ifelse(type %in% c("Hero", "Villain"), 
                       tolower(type), 
                       type.ah
      ) |> 
        factor(levels = c("hero", "villain")), # , "ah.hero", "ah.villain")),
      type = ifelse(type == "Anti-Hero" & !is.na(type.ah), NA, type) |> 
        factor(levels = c(
                 # "Hero", 
                 # "Villain", 
                 "Anti-Hero", # alts
      ...
      # Move "Generic Equipment" to front of villain dividers in Bore box?
      # Group Winter Guard together (Anti-Heroes)
      name = ifelse(
        str_detect(team, "Winter Guard") & name != "Winter Guard" & !is.na(team),
        paste("Winter Guard:", name),
        name
      ),
      # assign alternate names to villain forms of Anti-Heroes, but not alts
      name = case_when(
        type == "Anti-Hero" & str_detect(type.ah, "^villain") ~ str_replace(name, "^.*\\/\\s?", ""),
        type == "Anti-Hero" & str_detect(type.ah, "^hero")    ~ str_replace(name, "\\s?\\/.*$", ""),
        .default = name
    ...
    arrange(ks.exclusive, collection, type.ah, type, 
            str_rank(box), str_rank(name)
    ) |> 
    # derive page breaks
    mutate(
      page.break = case_when(
        ...
        as.logical(ks.exclusive) & 
          !duplicated(ks.exclusive) ~ TRUE,
        box == "Civil War" &
          !duplicated(box) ~ TRUE,
        .default = NA
      )
